...
type alias Model =
    ...
    -- "temp" is a list of products used to keep products on the page when they
    --  reach 0 quantity in the cart or they are dropped out of the favorites
    --  list
    , temp : List ProductId
    , order : Order
...
configuration =
    ...
    { nameWebsite = "Mini-Ichiba - Elm E-commerce Demo"
            ...
            , infoClosed = False
                ...
                |> (\m ->
                        ...
                        case stringToLocalStorage flags.localStorage of
                            Ok localStorage ->
                                injectLocalStorage localStorage m
...
updateMain msg model =
    case msg of
        ...
        ChangeLocalStorage localStorageAsString ->
            ( changeLocalStorage localStorageAsString model, Cmd.none )
...
updateErrors _ model =
    ( -- He we transfer errors to the warning system and the show the Top
      -- page so that users can interact with the app also in case of errors.
      case model.page of
        NotFound string ->
            { model
                | page = Top Nothing
...
atomLinkInternal attrs args =
    link
        ...
        -- TODO - Verify is it is ok to have "Nothing" here
...
atomPrice attrs fontSize price =
    row ([ tag "atomPrice", Font.size fontSize ] ++ attrs)
        [ text <| addThousandSeparator price
        , el [ Font.size <| fontSize // 3 * 2, alignBottom, moveUp 2 ] <| text "円"
        ]
...
atomQrCode : List (Attribute msg) -> ( String, String ) -> Element msg
atomQrCode attrs ( message, url ) =
    newTabLink []
        { url = url
        , label =
            column
                ([ tag "atomQrCode"
                 , width (fill |> minimum 140 |> maximum 160)
                 , Background.color <| rgba 1 1 1 0.9
                 , Font.color <| rgb 0 0 0
                 ]
                    ++ attrs
                )
                [ paragraph [ Font.size 13, paddingXY 10 0, moveDown 10, Font.center ] [ text <| message ]
                , html <|
                    (url
                        |> QRCode.fromString
                ...
                , paragraph [ Font.size 7, paddingXY 10 0, moveUp 10, Font.center ] [ text <| String.replace "https://" "" url ]
                ]
        }
...
atomTagNumber : List (Attribute msg) -> Bool -> Int -> Element msg
atomTagNumber attrs usePrimaryColor quantity =
    if quantity == 0 then
        none
            ...
            ([ tag "atomTagNumber"
             ...
             , Font.size 13
                ...
                ++ (if usePrimaryColor then
                        ...
                        [ attrWithContext <| \c -> Background.color c.palette.onSecondary
                        , attrWithContext <| \c -> Font.color c.palette.secondary
                        ]
                   )
                ++ attrs
            )
...
molMenuMain attrs model =
    ...
    in
    el
        ([ tag "molMenuMain", inFront viewSearchInputField ] ++ attrs)
    <|
        row
            [ spacing 15
            , centerX
            ...
            [ Input.button [ title "Search" ]
                { label =
                    atomIcon (attrsWithCounter True 0)
            ...
            , Input.button [ title "View Top" ]
                { label =
                    buttonEffect (attrsWithCounter True 0)
            ...
            , Input.button [ title "View Favorites" ]
                { label =
                    buttonEffect (attrsWithCounter True (List.length model.favorites))
            ...
            , Input.button [ title "View Cart" ]
                { label =
                    buttonEffect (attrsWithCounter True (List.length model.cart))
                ...
                ([ tag "molProductForStrip"
                 ...
                 , inFront <|
                    el
                        [ height fill
                        , width fill
                        , style "background-image" "linear-gradient(to bottom, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.4))"
                        ]
                        none
...
molQRcodes : Element msg
molQRcodes =
    elementWithContext <| \c -> wrappedRow [ centerX, spacing 20 ] <| List.map (\( message, url ) -> atomQrCode [ centerX ] ( message, url )) c.conf.qrCodeContent
                ...
                List.map
                    (\lang ->
                        Input.button
                            ...
                            , Font.size 12
                            , attrWithContext <| \c -> Font.color c.palette.onBackgroundDim
                            , attrWithContext <| \c -> Border.color c.palette.separator
                            , attrWithContext <|
                                \c ->
                                    Background.color <|
...
orgFooter =
    let
        attrs =
            attrsButton
                ++ [ attrWithContext <| \c -> Background.color (changeAlpha 0.1 c.palette.error)
                   , transition "background-color 0.2s"
                   , mouseOver
                        [ decorationWithContext <| \c -> Background.color (changeAlpha 0.2 c.palette.error)
                        ]
                   , attrWithContext <| \c -> Font.color c.palette.error
                   , attrWithContext <| \c -> Border.color c.palette.error
                   , Border.width 1
                   ]
    in
    column [ tag "orgFooter", attrWidth, centerX, Font.center, paddingXY 30 50, spacing 40 ]
        ...
        , column [ padding 10, centerX, spacing 10, Font.size 13 ]
            [ paragraph [ attrWithContext <| \c -> Font.color c.palette.error ] [ text "Danger Zone" ]
            , wrappedRow
                [ spacing 10 ]
                [ Input.button []
                ...
                (\product ->
                    ...
                    linkWrapperForProduct product
                        (atomImageProduct
                            [ width <| px 80
                            , if quantity > 1 then
                                inFront <| atomTagNumber [ alignRight, moveUp 3, moveRight 3 ] False quantity
                        ...
                        , label =
                            column [ spacing 10 ]
                                [ el [ width (fill |> minimum 80), moveDown 5 ] <|
                                    atomIcon (attrsWithCounter False (List.length model.cart) ++ [ centerX ])
                                        { shape = FeatherIcons.shoppingCart
                                        , color = .onSecondary
                                        , size = 40
                                        , fill = False
            ...
            ++ (if model.favorites == [] then
                    ...
                    Input.button []
                        ...
                        , label =
                            el [ width (fill |> minimum 80), moveDown 5 ] <|
                                atomIcon
                                    (attrsWithCounter False (List.length model.favorites) ++ [ centerX ])
                                    { shape = FeatherIcons.heart
                                    , color = .onSecondary
                                    , size = 40
                                    , fill = False
...
inFrontInfoModal =
        ...
        , inFront <|
            ...
            <|
                column
                    ...
                    [ paragraph [ Font.size 20 ] [ text "Mini Ichiba" ]
                    , paragraph
                        []
                        [ text "This is a small E-commerce demo applications built with "
                        , newTabLink [ Font.underline ] { url = "https://elm-lang.org/", label = text "Elm" }
                        , text "."
                        ]
                    , paragraph [] [ text "It is made of" ]
                    , html <|
                        Html.pre []
   ...
   15 lines of HTML
    0 lines of CSS"""
                    ...
                    , paragraph [] [ text "Elm is a domain-specific purely functional language that compiles to JavaScript" ]
                    , paragraph [] [ text "Elm guarantees:" ]
                    , column [ spacing 2 ]
                        [ paragraph [] [ text "• No runtime exception." ]
                        , paragraph [] [ text "• 100% immutable data." ]
                        , paragraph [] [ text "• 100% pure functions, also in dependencies." ]
                        , paragraph [] [ text "• 100% type inference." ]
                        , paragraph [] [ text "• ", newTabLink [ Font.underline ] { label = text "Etc.", url = "https://discourse.elm-lang.org/t/what-are-your-favorite-things-about-elm/6947?u=lucamug" } ]
                        ]
                    , paragraph [] [ text "These characteristics make the code robust, performant, and simpler to comprehend and maintain." ]
                    , paragraph [ Font.size 20 ] [ text "Characteristics of this demo" ]
                    , column [ spacing 4 ]
                        ...
                        , paragraph [] [ text "• Search and Sort" ]
...
attrsWithCounter : Bool -> Int -> List (Attribute msg)
attrsWithCounter usePrimaryColor counter =
    [ tag "attrsWithCounter", onRight <| atomTagNumber [ moveUp 10, moveLeft 10 ] usePrimaryColor counter ]
...
changeLocalStorage localStorageAsString model =
    case stringToLocalStorage localStorageAsString of
        Ok localStorage ->
            injectLocalStorage localStorage model

        Err _ ->
            model


injectLocalStorage : LocalStorage -> Model -> Model
injectLocalStorage localStorage model =
    { model
