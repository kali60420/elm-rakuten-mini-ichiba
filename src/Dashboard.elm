...
main =
    Browser.document
        { init = init
        , view = \model -> { title = "Meta Mini-Ichiba", body = view model }
        , update = update
            ...
            <|
                column [ spacing 15, scrollbarY ]
                    ([ column
                        ...
                        , centerX
                        , spacing 2
                        , Main.attrWithContext <| \c -> Background.color c.palette.onBackground
                        , Main.attrWithContext <| \c -> Font.color c.palette.background
                        ]
                        [ paragraph [ centerX, Font.center ] [ text "Mini Ichiba" ]
                        , paragraph [ Font.size 12, Font.center, centerX ] [ text "dashboard" ]
                        ]
                     , map MsgMain <|
                        menuIcon
                        ...
                        ++ List.map
                            (\device ->
                                ...
                                menuIcon deviceAttrs_.icon
                                    deviceAttrs_.name
                                    (ChangeDevice device)
                                    (model.device == device && model.page /= DesignSystem)
                            )
                            devices
                        ++ [ newTabLink [ centerX ]
                                { label =
                                    column [ centerX, spacing 5 ]
                                        [ Main.atomIcon [ centerX ] { color = .onBackground, fill = False, shape = FeatherIcons.code, size = 30 }
                                        , paragraph [ Font.size 12, centerX, Font.center, spacing 1 ] [ text "source code" ]
                                        ]
                                , url = "https://git.rakuten-it.com/projects/ELM/repos/elm-rakuten-mini-ichiba/browse"
                                }
                           ]
                    )
        , inFront <|
...
viewDesignSystem modelReal =
    ...
    <|
        paragraph [ Font.size 30, Font.center ] [ text <| pageToString DesignSystem ]
            ...
            ++ sectionB "orgHeader" (Main.orgHeader modelFull.page)
            ...
            ++ sectionB "orgFooter" Main.orgFooter
            ...
            ++ sectionA "molMenuMain" (Main.molMenuMain [] modelFull)
            ...
            ++ sectionA "molMenuMain (Functional)" (Main.molMenuMain [] modelReal)
            ++ sectionA "molMenuMain (Top)" (Main.molMenuMain [] { modelFull | page = Main.Top Nothing })
            ++ sectionA "molMenuMain (Cart)" (Main.molMenuMain [] { modelFull | page = Main.Cart Nothing })
            ++ sectionA "molMenuMain (Favorites)" (Main.molMenuMain [] { modelFull | page = Main.Favorites Nothing })
            ++ sectionA "molMenuMain (Search)" (Main.molMenuMain [] { modelFull | page = Main.pageFromSearchQuery (Just "search query") modelFull.page })
            --
            -- ATOMS
            ++ separation "Atoms"
            ...
            ++ sectionA "atomQrCode" (Main.atomQrCode [] ( "test", "test" ))
            ++ sectionA "atomImageProduct" (Main.atomImageProduct [] 10 p1)
            ++ sectionA "atomTagNew" (Main.atomTagNew [] "TAG")
            ++ sectionA "atomTagNumber (primary)" (Main.atomTagNumber [] True 99)
            ++ sectionA "atomTagNumber (non-primary)" (Main.atomTagNumber [] False 99)
